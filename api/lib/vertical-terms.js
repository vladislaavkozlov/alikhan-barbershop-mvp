...
export const PHRASES = {
  barbershop: {
    ...
    // (13.09.2026, находка владельца): флаг убирает человека из состава расписания, а
    // живые записи к нему остаются в базе и пропадают с глаз вместе с ним
  ...
  clinic: {
    // Уведомления на телефон (Окно 73, 28.08.2026)
    'push.hintOn': 'Включены на этом устройстве. Телефон покажет уведомление о новом приёме, даже когда кабинет закрыт',
    'push.hintOff': 'Сейчас выключены. Включите, чтобы узнавать о новых приёмах, не открывая кабинет',
    'push.iosInstall': 'Чтобы получать уведомления о приёмах на айфон, откройте этот кабинет через кнопку «Поделиться» внизу Safari, выберите «На экран Домой», а потом запустите приложение с экрана и включите уведомления здесь же',
    'push.enabled': 'Готово. Телефон будет показывать уведомления о новых приёмах',
    ...
    'bell.ariaShort': 'Уведомления о приёмах и графике',
    'master.yearHint': 'Год - только мои приёмы. Праздники - официальный календарь РФ',
    'master.commission': 'Моя комиссия за приём',
    ...
    'renew.noPhone': 'Срок возврата у этого приёма не спрашиваем - у пациента не указан телефон, напоминать некому. Впишите номер в поле «Телефон» выше, если хотите вести его историю и видеть его в «Недополученной выручке»',
    ...
    'team.acceptsOffHasBookings': 'К «{name}» записаны пациенты: впереди приёмов {count}, ближайший {when}. Снимете приём пациентов - строка пропадёт из расписания вместе с этими приёмами, а пациентов никто не предупредит. Сначала перенесите приёмы на другого сотрудника или отмените их',
    'team.acceptsOffShort': 'Не сохранено: впереди приёмов {count}. Решите, что с ними делать',
    'team.acceptsOffCheckFailed': 'Не удалось проверить будущие приёмы - связь с сервером подвела',
    'team.acceptsOffKeep': 'Оставить приём пациентов',
    ...
    'analytics.renewDefaultNote': 'Остальным поставлен месяц по умолчанию - это нормальный ответ врача, но чем таких меньше, тем точнее «Недополученная выручка» в «Финансах»',
    ...
    'missed.repliedNoBooking': 'ответил «да», но на приём ещё не записался - звоните',
    'missed.rebooked': 'сам записался на приём {date} - звонить не надо',
...
// Оформление кабинета по вертикали (30.08.2026). Тот же принцип, что у словаря:
// «клиника» - это не только слово «врач» вместо «мастера», но и другой визуальный
// язык. Барбершоп остаётся на том оформлении, которое у него было всегда, и его
// значение здесь названо явно (`default`), а не пропуском - пропуск читался бы как
// «тему забыли», а это осознанный выбор.
//
// Почему тема живёт здесь, а не колонкой у арендатора: цвет кабинета - свойство
// РОДА заведения, ровно как склонения. Второй барбершоп получит вид Алихана,
// вторая клиника - вид клиники, и ни одной строки под конкретного клиента писать
// не придётся. Понадобится оформление под отдельного арендатора - точка расширения
// одна, эта таблица.
...
export function themeFor(vertical) {
  return THEMES[resolveVertical(vertical)] ?? 'default';
}

// Ответ роута GET /tenant/appearance. Наружу уходят только вертикаль, слова и имя
// темы: ни названия арендатора, ни доменов, ни чего-либо про его клиентов. Копия
// глубокая - иначе правка ответа в одном запросе испортила бы словарь для всех
// следующих
export function appearanceFor(vertical) {
  const resolved = resolveVertical(vertical);
  return {
    vertical: resolved,
    theme: themeFor(resolved),
