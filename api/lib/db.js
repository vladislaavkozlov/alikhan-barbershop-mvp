...
// недополученная выручка). Раньше каждая брала своё соединение из пула и это было
...
// Единственная дверь к базе МИМО арендатора - и она открывается только в справочники
// без замка. Нужна она ровно для одного: определить, чей это запрос, до того как
// контекст арендатора вообще существует. Любая другая таблица через неё недоступна не
// по договорённости, а потому что замок в базе её всё равно не отдаст без контекста.
//
// Список умышленно короткий и закрытый:
//   tenants         - справочник арендаторов (миграция 058 оставила его без замка);
//   tenant_channels - боты арендаторов (миграция 062). Входящее обновление от
//                     Telegram приходит на секретный адрес, и именно по этой таблице
//                     определяется, чей это бот. Клиентских данных в ней нет, только
//                     конфигурация канала.
const REGISTRY_TABLES = /\bFROM\s+(tenants|tenant_channels)\b/i;

export async function registryQuery(text, params) {
  if (!REGISTRY_TABLES.test(text)) {
    throw new Error('registry_query_scope: эта дверь открывается только в справочники без замка');
  }
