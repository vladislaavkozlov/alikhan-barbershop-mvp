...
export function buildPublicMasters(masterRows, serviceRows = []) {
  ...
  for (const r of serviceRows) {
    // isTop (20.08.2026, миграция 054) - по этому признаку сайт делит мастеров на два
    // тарифа в форме записи. Публичность признака осознанная: цена топ-мастера и так
    // видна клиенту в той же строке, скрывать причину, по которой она выше, незачем.
    masters.get(r.master_id)?.services.push({ id: r.id, name: r.name, price: r.price, durationMin: r.duration_min, isTop: r.is_top === true });
  }
