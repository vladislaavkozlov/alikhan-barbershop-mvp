...
// рабочим - обещал то, чего не происходит. Тогда тумблер сделали неактивным.
//
// 16.08.2026 это вернулось другой стороной: "при нажатии на ползунок не становится
// активной кнопка сохранить изменения". Неактивный тумблер выглядел обычным, на
// нажатие не отвечал и причину нигде не объяснял. Теперь настройка переключается
// всегда, а несбыточное обещание снимает ПОДПИСЬ под ней - забота та же, способ
// другой: сказать словами вместо мёртвого контрола.
test('тумблер витрины переключается всегда, а его бессилие объяснено подписью', async () => {
  const root = new URL('../', import.meta.url);
  const team = await readFile(new URL('assets/crm-team.js', root), 'utf8');
  ...
  // Снят с приёма - подпись прямо говорит, что на сайте его нет и когда появится
  assert.match(markup, /offDuty\s*\n?\s*\?\s*'Сотрудник снят с приёма[^']*'/);
  // и сам тумблер больше не блокируется этим состоянием
  assert.doesNotMatch(markup, /name: 'publicProfileEnabled'[\s\S]*disabled: offDuty/);
});
