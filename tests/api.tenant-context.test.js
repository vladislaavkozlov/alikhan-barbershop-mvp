...
test('каждый запрос сервера идёт внутри контекста арендатора', () => {
  ...
  //
  // Хвост аргументов намеренно не фиксируется (ослаблено 24.08.2026, Этап B):
  // охраняется здесь обёртка - что запрос идёт внутри контекста арендатора и что
  // долгий ответ пишет в res, а обычный в буфер. Этап B добавил обработчику пятый
  // аргумент (арендатор нужен роуту словаря), а самой обёртке - вертикаль, чтобы
  // серверные тексты знали, на каком языке говорить
  assert.match(
    serverSource,
