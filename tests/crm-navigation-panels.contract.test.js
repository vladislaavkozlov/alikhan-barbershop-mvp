...
test('все три кабинета подключают общий слой навигационных панелей', async () => {
  ...
  // 7, а не 8: 20.08.2026 из «Уведомлений» убрана карточка «Заявки на изменение
  // графика» вместе со всем механизмом заявок. Раздел теперь показывает ленту записей
  // клиентов (#notifCenter) - она осознанно НЕ завёрнута в details.staff-card: карточка
  // нужна там, где в разделе несколько блоков и их сворачивают, а здесь лента и есть
  // единственное содержимое раздела - прятать её за лишним раскрытием нечего.
  // До этого: 8, а не 12 - 17.08.2026 убраны блок «Управление скидками» в «Финансах»
  // и три макетные карточки мастеров в «Команде» (их рисует renderTeam из GET /staff)
  // 9, а не 7: 22.08.2026 (Окно 59) добавились «Недополученная прибыль» в «Финансах» и
  // «Сроки возврата проговорены» в «Аналитике» - обе сворачиваемые, тем же компонентом
  assert.equal((pages[0].match(/<details class="staff-card/g) || []).length, 9);
...
test('фон закрытых, наведённых и открытых панелей строит единый уровень глубины', async () => {
  ...
  assert.match(css, /--owner-panel-quiet: var\(--panel-bg-collapsed\);/);
  assert.match(css, /--owner-panel-hover: var\(--panel-bg-light\);/);
  assert.match(css, /--owner-panel-active: var\(--panel-bg\);/);
