...
// Единый порядок показа услуг во ВСЕХ формах выбора (Влад, 16.08.2026): стрижка →
// борода → комплекс → бритьё → фирменная окантовка → тонировка → воск → СПА уход.
// Источник правды один - порядок объявления SERVICES выше, отдельного списка id
// нет специально, чтобы он не разъехался с каталогом. Бэкенд сортирует тем же
// порядком через колонку services.sort_order (api/migrations/049_service_sort_order.sql),
// эта клиентская сортировка - страховка: фронт (GitHub Pages) и бэкенд (Amvera)
// деплоятся раздельно, между двумя деплоями API какое-то время отдаёт старый
// алфавитный порядок, и без неё формы показывали бы его.
export const SERVICE_ORDER = SERVICES.map((s) => s.id);

// Незнакомая услуга (заведена в базе позже, чем этот файл) уходит в хвост, а не
// наверх - sort стабилен, поэтому такие сохраняют относительный порядок источника.
export function serviceOrderIndex(serviceId) {
  const index = SERVICE_ORDER.indexOf(serviceId);
  return index === -1 ? SERVICE_ORDER.length : index;
}

// Копия списка в каноническом порядке. getId - потому что услуга приезжает в трёх
// формах: каталог (`{id}`), строки master_services (`{serviceId}`) и состав брони.
export function sortByServiceOrder(list, getId = (item) => item.id) {
  return [...list].sort((a, b) => serviceOrderIndex(getId(a)) - serviceOrderIndex(getId(b)));
}
