...
## Пойманная регрессия (фаза 10, исправлена в том же коммите)

`crm-walkin.js` (фаза 9) импортировал `renderLiveProof` из `./crm-auth.js`, потому
что на момент фазы 9 функция физически ещё жила там. Фаза 10 переносит
`renderLiveProof` в `crm-dashboard.js` - `crm-auth.js` стал ПОЛУЧАТЕЛЕМ импорта
(`import { renderLiveProof } from './crm-dashboard.js'`), а не его источником. ES-
модули не ре-экспортируют импортированные привязки автоматически - `crm-walkin.js`
продолжал просить у `crm-auth.js` то, чего там больше нет, что дало жёсткий
`SyntaxError: ... does not provide an export named 'renderLiveProof'` уже на этапе
загрузки модуля (весь граф модулей на странице не инициализировался вообще, отсюда
широкая регрессия - 11/26 живых проверок, а не точечная). Исправлено: у
`crm-walkin.js` импорт `renderLiveProof` переставлен на прямой источник -
`crm-dashboard.js`, туда же, где функция теперь реально определена. Урок: когда
домен-потребитель (walk-in) перенесён РАНЬШЕ домена-владельца функции (дашборд),
после переноса владельца нужно сверить импорты потребителя на актуальный источник,
а не полагаться на re-export через промежуточный файл.
