...
//   OWNER_LOGIN=aliovsad OWNER_PIN=<пароль> node tools/diag-2026-08-17-kabinety.mjs
const env = (name) => {
  const value = process.env[name];
  if (!value) {
    console.error(`Не задан доступ ${name}. Пример: ${name}=<значение> node tools/diag-2026-08-17-kabinety.mjs`);
    process.exit(1);
  }
  return value;
};
...
//   OWNER_LOGIN=<логин> OWNER_PIN=<пароль> node tools/diag-2026-08-17-kabinety.mjs
