...
try {
  ...
  await step('бот здоровается языком заведения даже с незнакомцем', async () => {
    // Найдено на живом боте 01.09.2026: человеку без приглашения клиника писала
    // «напоминания о визите» - слово было зашито мимо словаря вертикали
    const res = await runInTenant(2, () => processUpdate({ message: { text: '/start', chat: { id: 771111 } } }, BOT, 'clinic'), 'clinic');
    assert.equal(res.action, 'start_without_payload');
    const text = sentTexts().at(-1);
    assert.match(text, /о приёме/i, `клиника говорит не своими словами: ${text}`);
    assert.doesNotMatch(text, /о визите/i, `в текст клиники попало барбершопное слово: ${text}`);
  });
