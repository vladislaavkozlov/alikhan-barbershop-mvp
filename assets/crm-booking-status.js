...
// есть на владельце (Окно 36) и у администратора (Окно 55).
//
// В кабинете мастера контролов статуса НЕТ - ни радио, ни кнопки (карточка записи
// только на просмотр с 22.08.2026, коммит d9efed5), и с правкой Влада 22.08.2026
// (Окно 59) роут PATCH /bookings/:id/status роль master больше не пускает вовсе:
// отметка «Обслужен» - это фиксация сделки, которая идёт в выручку и в зарплату того
// же мастера, и ставит её администратор, владелец или управляющий. Гейт по
// #bk-status-note остаётся признаком «на этой странице статус реально пишется», а не
// ролевой проверкой - настоящая проверка живёт на сервере.
//
