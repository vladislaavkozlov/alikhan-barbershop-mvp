...
// Правка 17.08.2026 (Влад): «в Команде нужно поставить мастеру график с любого
// времени по любое, 00:00-23:59, а не 10:00-20:00 как сейчас». Этот список -
// единственный источник опций ВСЕХ time-пикеров CRM (график по дням недели, разовая
// правка дня, перерыв, заявка мастера на отгул, время записи в форме), поэтому
// «круглосуточно» достаточно открыть здесь один раз - иначе получилось бы окно
// графика, внутри которого нельзя выбрать перерыв или время записи.
// 23:59 отдельным последним значением, а не 24:00: смена живёт внутри одних суток
// (schedule_shifts.date + start < end), «до полуночи» выражается именно 23:59.
export const SHOP_TIME_OPTIONS = (() => {
  const opts = [];
