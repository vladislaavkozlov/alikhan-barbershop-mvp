...
// Правка 22.08.2026 (увольнение): employed раньше здесь не проверялся вовсе, и обе
// ветки давали неверную цифру. Уволенный с невыключенной галкой услуг оставался в
// блоке зарплат наравне с работающими (тот же промах, что чинили в календаре
// 20.08.2026 - см. assets/crm-calendar.js). А если галку снять, из "Финансов"
// пропадал и его заработок за уже отработанные периоды - деньги были, показать
// нечем. Поэтому правило теперь про обоих:
//   работает        - карточка есть всегда, даже с нулём (пустая строка это ответ)
//   уволен          - карточка есть, только если в загруженном окне у него были
//                     ОПЛАЧЕННЫЕ визиты. Иначе через год блок наполовину состоял бы
//                     из призраков (решение Влада, 22.08.2026)
// mastersWithPaidVisits - Set id из уже посчитанных paidBookings. Не передан (данные
// не загрузились) - показываем только действующий состав, а не всех подряд.
export function payrollStaff(staffList, mastersWithPaidVisits) {
  return (staffList ?? []).filter((s) => {
    if (!isEmployed(s)) return !!mastersWithPaidVisits?.has(s?.id);
    return !!s?.providesServices;
  });
}

// ── Уволенные (22.08.2026) ────────────────────────────────────────────────────
// Увольнение в CRM - это архив, а не удаление: строка сотрудника остаётся, флаг
// employed уходит в false, дата фиксируется в employmentEndedAt (миграция 055).
// Поэтому «человек в команде» и «человек существовал» - разные вопросы, и путать
// их нельзя ни в одном разделе.
export function isEmployed(staff) {
  return staff?.employed !== false;
}

// Дата увольнения человеческим видом: "15.06.2026". Пустая строка, если уволен до
// появления колонки (дату таким не выдумываем - её честно не знает никто)
export function formatEmploymentEnd(dateStr) {
  const parts = String(dateStr ?? '').split('-');
  return parts.length === 3 ? `${parts[2]}.${parts[1]}.${parts[0]}` : '';
}

// Подпись под именем уволенного - одна на все разделы, чтобы «Команда», «Финансы» и
// «Аналитика» не разошлись формулировками
export function firedLabel(staff) {
  const date = formatEmploymentEnd(staff?.employmentEndedAt);
  return date ? `Не работает с ${date}` : 'Не работает';
}
