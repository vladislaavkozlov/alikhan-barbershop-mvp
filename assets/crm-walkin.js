  ...
  function openForWalkin(masterId, masterName, options = {}) {
    ...
    if (hasRebookUi) {
      ...
      if (rebookMode) {
        ...
        // не переданы явно (options.date/options.startTime - клик по слоту календаря
        // их всегда передаёт) - владелец меняет на любое реальное свободное,
        // доступность проверяет сервер при сохранении.
        // 17.08.2026 - зажим в 10:00-20:00 снят вместе с суточным SHOP_TIME_OPTIONS
        // (crm-widgets.js): у мастера с ночным графиком форма подставляла 10:00 вместо
        // реального «сейчас», то есть предлагала время, когда он уже не работает.
        // Верхняя граница - последняя четверть суток: 23:59 как СТАРТ записи бессмыслен
        const now = new Date();
        const roundedMin = Math.min(23 * 60 + 45, Math.ceil((now.getHours() * 60 + now.getMinutes()) / 15) * 15);
