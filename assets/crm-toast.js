...
const ERROR_TEXT = {
  ...
  // Мультиарендность (24.08.2026): сервер не узнал домен, с которого пришёл запрос.
  // Для сотрудника это значит, что кабинет открыт по неправильному адресу
  unknown_tenant: 'Этот адрес сервер не обслуживает - откройте кабинет по рабочей ссылке',
  // Живое обновление временно идёт опросом, а не потоком (инцидент 24.08.2026).
  // Человек этот код на экране не увидит: crm-live.js молча переключается на опрос,
  // но перевод нужен, иначе наружу выйдет техническая строка
  events_disabled: 'Живое обновление идёт с небольшой задержкой - данные обновляются сами',
  ...
  // Окно 72 (28.08.2026): пять промахов подряд - вход на паузе, чтобы пароль
  // нельзя было подобрать перебором
  too_many_attempts: 'Слишком много неудачных попыток. Попробуйте снова через 15 минут',
  email_and_pin_required: 'Введите логин и пароль',
  // Раздел выключен у этого арендатора (Этап B, 24.08.2026): сервер отдаёт 404, а не
  // 403 - для него такого раздела не существует, а не «нет прав»
  module_disabled: 'Этот раздел не подключён',
  invalid_pin: 'Пароль - минимум шесть знаков, без пробелов по краям',
  email_in_use: 'Этот логин уже занят другим сотрудником',

  staff_not_found: 'Сотрудник не найден - возможно, его уже удалили',
  // Увольнение (22.08.2026). Замок повторяет серверный guardAccountLockout: владельца
  // и себя самого из состава не убрать
  employment_locked: 'Этого сотрудника нельзя уволить: владельца и самого себя из состава не убирают',
  invalid_employment: 'Не удалось изменить состав команды - повторите попытку',
  unknown_master: 'Мастер не найден',
  invalid_staff_data: 'Проверьте имя, логин и роль сотрудника',
  ...
  invalid_price: 'Цена услуги должна быть целым числом больше нуля',
  invalid_pct: 'Ставка должна быть числом от 0 до 100',

  invalid_service_name: 'У услуги должно быть название - не длиннее 120 знаков',
  invalid_sort_order: 'Порядок показа - целое число, ноль или больше',
  nothing_to_update: 'В услуге нечего менять - ни одно поле не передано',
  service_in_use: 'Услугу нельзя удалить: она уже стоит в записях. Уберите её из этих записей или оставьте в каталоге',
  invalid_category: 'Категория услуги бывает только основной или комплексной',
  ...
  // Окно 59 (22.08.2026) - срок обновления стрижки при закрытии визита. Тексты
  // объясняют, что делать, а не констатируют отказ: поле обязательное, и мастер должен
  // понять из фразы, что «не обсуждали» - законный выход, а не тупик
  renew_required: 'Укажите, через сколько клиенту прийти снова - или выберите «Не обсуждали»',
  invalid_renew_days: 'Срок возврата должен быть от 7 до 365 дней',
  invalid_renew_reason: 'Выберите, почему у клиента такой срок',
  invalid_period: 'Период выбран неверно - начало позже конца',
  invalid_kind: 'Такого списка клиентов нет',
  ...
  // Бот заведения не подключён: ссылку-приглашение выдать не из чего (Волна 1)
  channel_not_configured: 'Бот не подключён - ссылку выдать нельзя',
  // Форма записи на сайте клиента не загрузилась (01.09.2026). Кабинет этот код
  // не показывает, но словарь общий и сторож требует перевода для каждого
  widget_not_found: 'Форма записи временно недоступна',
  ...
  // Источник клиента (17.08.2026). Оба случая - не ошибка человека за экраном:
  // в форме канал выбирается из списка, поэтому сюда можно попасть только запросом
  // мимо неё (старая открытая вкладка, прямой вызов API). Пишем прямо, что канал не
  // распознан, вместо технического ключа
  invalid_client_source: 'Источник клиента передан неверно',
  unknown_client_source: 'Такого источника клиента нет в списке - выберите из списка',
  ...
  // 17.08.2026 - раньше на все три случая была одна общая фраза «проверьте время», и
  // по ней нельзя было понять, что именно не так (замечание Влада по живому экрану).
  // Форма графика обычно ловит это ещё до отправки и пишет фразу с днём недели и
  // часами (firstWeeklyProblem, assets/crm-schedule-editor.js) - эти тексты нужны для
  // ответов сервера: заявка мастера на смену графика и любой другой клиент
  invalid_weekly_changes: 'График передан неверно - обновите страницу и попробуйте снова',
  missing_work_time: 'Укажите, с какого и до какого часа мастер работает',
  missing_break_time: 'Укажите время перерыва целиком - с какого и до какого часа',
  work_end_before_start: 'Конец рабочего дня должен быть позже начала',
  break_end_before_start: 'Конец перерыва должен быть позже начала',
  break_outside_work: 'Перерыв должен быть внутри рабочего дня мастера',
