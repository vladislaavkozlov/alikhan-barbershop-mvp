...
function cellInnerHtml(cell) {
  if (cell.missing) return '<span class="sm-cell-off">—</span>';
  ...
  return `<span class="sm-cell-hours">${escapeHtml(cell.startTime)}–${escapeHtml(cell.endTime)}</span>
    ${breakLabel}
    <span class="sm-cell-load"><span class="sm-cell-pct">${cell.loadPct}%</span>${cell.bookingCount ? ` · ${cell.bookingCount}` : ''}</span>`;
}
