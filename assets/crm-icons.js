...
// ── Иконки по вертикали (31.08.2026, находка Влада: «иконки в кабинете Карины в
// стиле Алихана») ─────────────────────────────────────────────────────────────
// Ножницы выше - осознанный знак БАРБЕРШОПА, и в кабинете врача-ортодонта они
// стояли и в меню («Команда»), и в карточке сотрудника («Процедуры и время»):
// клиника, которая стрижёт. Словарь вертикали уже переводит слова (Этап B), рисунок
// оставался барбершопным. Здесь то же самое делается с рисунком: набор тот же
// line-art, тот же viewBox 20x20 и тот же STROKE, меняется только метафора.
//
// Правило отката общее с словарём: вертикаль, которой в таблице нет, получает
// барбершопную иконку, то есть у Алихана не меняется ровно ничего.

// Стетоскоп - врач. Не «человек в халате»: соседний пункт «Пациенты» уже занят
// парой силуэтов (ICON_CLIENTS), и второй силуэт рядом читался бы тем же разделом
export const ICON_TEAM_CLINIC = `<svg viewBox="0 0 20 20" ${STROKE} xmlns="http://www.w3.org/2000/svg"><path d="M5.4 2.6v4.2a3.6 3.6 0 0 0 7.2 0V2.6"/><path d="M3.9 2.6h1.5M12.6 2.6h1.5"/><path d="M9 10.4v2a3.3 3.3 0 0 0 6.6 0v-.7"/><circle cx="15.6" cy="9.9" r="1.9"/></svg>`;

// Медицинский крест на карточке - процедура. Ценник (ICON_CATALOG) остаётся у
// раздела меню: он про прайс и одинаково верен для стрижки и для приёма, а здесь
// нужен знак самой процедуры внутри карточки сотрудника
export const ICON_SERVICES_CLINIC = `<svg viewBox="0 0 20 20" ${STROKE} xmlns="http://www.w3.org/2000/svg"><rect x="3" y="3.5" width="14" height="13" rx="2.5"/><path d="M10 6.9v6.2M6.9 10h6.2"/></svg>`;

// Таблица подмен: вертикаль → что заменить. Нет вертикали в таблице или нет ключа -
// отдаём барбершопную иконку, поэтому новый арендатор без своей строки открывается
// в исходном виде, а не с пустым местом вместо иконки
const VERTICAL_ICONS = Object.freeze({
  clinic: { team: ICON_TEAM_CLINIC, services: ICON_SERVICES_CLINIC },
});

const DEFAULT_ICONS = Object.freeze({ team: ICON_TEAM, services: ICON_SERVICES });

export function verticalIcon(key, vertical) {
  return VERTICAL_ICONS[vertical]?.[key] ?? DEFAULT_ICONS[key] ?? '';
}
